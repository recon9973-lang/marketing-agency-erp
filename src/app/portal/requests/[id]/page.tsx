...
export default async function PortalRequestDetailPage({
  ...
  // 접근 제어: 자기 거래처 요청만
  if (!request || request.clientId !== user.clientId) notFound();
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 p-6 space-y-4">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="min-w-0">
            ...
            <div className="flex items-center gap-3 mt-2 text-xs text-slate-400 flex-wrap">
              ...
              <span>작성: {request.author.name}</span>
              {request.assignee && (
                <span className="text-sky-500">담당: {request.assignee.name}</span>
              )}
            </div>
          </div>
        </div>
        ...
        {/* 본문 */}
        <div className="mt-4 p-4 bg-slate-50 rounded-xl text-sm text-slate-700 leading-relaxed whitespace-pre-wrap">
          {request.body}
        </div>
