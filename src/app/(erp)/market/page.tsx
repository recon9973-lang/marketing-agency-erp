...
export default async function MarketPage({
  ...
  searchParams: Promise<{ region?: string; specialty?: string; brand?: string; leadId?: string }>;
}) {
  const user = await getCurrentUser();
  if (!user) redirect("/login");
  const { region, specialty, brand, leadId } = await searchParams;
  ...
  return (
    <section className="space-y-4">
      ...
      />
      <MarketAnalysis presetRegion={region ?? ""} presetSpecialty={specialty ?? ""} presetBrand={brand ?? ""} presetLeadId={leadId ?? ""} />
