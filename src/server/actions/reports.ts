...
export async function generateMonthlyReport(input: unknown): Promise<ActionResult<{ id: string }>> {
  return runAction(async () => {
    ...
    // 집계·저장은 공유 빌더에 위임(자동 배치와 동일 로직).
    const built = await buildMonthlyReportDraft({ clientId, clientName: client.name, reportingMonth, authorId: user.id });

    const meta = await requestMeta();
    await db.$transaction(async (tx) => {
      await recordAudit(tx, { actorId: user.id, action: "report.autoGenerate", targetType: "Report", targetId: built.id, afterState: built.stats, ...meta });
    });
