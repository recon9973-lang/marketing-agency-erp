      ...
      <tbody>
        {rows.map((r) => (
          <tr key={r.id} className="border-t">
            ...
            <td className="text-right">{/* /clients/[id] 상세 라우트는 §4 UI 배선에서 추가 예정 — typedRoutes 통과용 캐스트 */}
            <Link href={`/clients/${r.id}` as Route} className="text-xs text-[#533afd] underline">상세</Link></td>
          </tr>
        ))}
      </tbody>
    </table>
