...
## 진행 현황 (2026-07-17 업데이트 — 이식 착수·4/5 완료)

TS 이식 방법 확립(파이썬 목→골든 픽스처→TS→골든 테스트) 후 **4개 모듈을 이식·화면 배선 완료**. 전부 `erp-v1` 배포, `src/server/geo-studio/*`.
...
| **M1 스캐너** | ⏳ 미착수 | — | (geo-engine dedup 대상) |

- **골든 71건 통과**(파이썬 원본과 수치·문자열 완전 일치 — 정확 float·k-means·JSON-LD·TA). `schema.prisma` 무변경 → 다른 ERP 업그레이드와 **충돌 0**.
...
1. **M1 스캐너 → `geo-engine` dedup**: ERP에 이미 4-AI 인용탐지 엔진 존재 → 신규 화면이 아니라 **기존 엔진과 통합/정리**. 기존 코드 수정이라 조율 필요.
2. **P0 라이브 LLM 레이어**: 이식 4모듈의 목→실제 4-AI 호출 전환(`integrations/llm` 단일화 + Gemini 이름 조율 §5). 공용 provider 코드 → 조율 필수.
3. (선택) **DB 영속화**: 현재 모든 화면 계산 전용(미저장). 캠페인·CEP·여정 저장이 필요하면 `schema.prisma` 신규 모델(§6) — 이것도 스키마 접점이라 조율.

> **권고**: 순수 신규 이식(충돌 0)은 여기서 매듭. M1/P0/DB는 스키마·공용코드를 건드리므로 **다른 ERP 업그레이드 머지 후** 조율해 진행.
