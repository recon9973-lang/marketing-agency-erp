...
| C5 | **내보내기** — 표준 `.ics`(구글·애플·아웃룩 공용). 구글 OAuth 대신 무의존 파일 내보내기 | ✅ |

### C5 구현 메모(.ics 내보내기)
- 결정: 구글 전용 OAuth 대신 **표준 iCalendar(.ics)** — 외부 의존·인증 없이 구글·애플·아웃룩 어디든 가져오기/구독. 재설계 원칙("자체=본체, 외부=내보내기")에 부합.
- `server/calendar/ics.ts`: 순수 빌더(CRLF·RFC5545 텍스트 이스케이프·75옥텟 폴딩[멀티바이트 안전]). 7 tests.
- `/calendar/export` 라우트: 로그인 사용자 스코프 이벤트 [-30d,+180d] → `text/calendar` 첨부.
- UI: 캘린더 상단 "내보내기(.ics)" 버튼. 외부 캘린더 카드는 "가져오기 가능·양방향은 준비중"으로 정직화.
- 후속(opt-in): 실시간 양방향 구글 연동은 별도.
